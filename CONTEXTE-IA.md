...
| P1 | Fondation PvP (`GameMode`, join, tokens) | Fait |
| P1 | Routes shots/boards PvE et PvP | À faire |
| P2 | Sauvegarde fichier/DB, stats avancées | À faire |
...
Décisions clés déjà actées : persistance InMemory en singleton ; validation FluentValidation côté API ; dual-mode **VsComputer** (défaut) et **VsPlayer** (ADR 0006) ; `GameDto` sans positions de navires.
...
|-------|--------|------------------|---------------|
| Mode de jeu | VsComputer + VsPlayer (fondation) | Auth comptes utilisateurs | PvP via tokens par partie et header `X-Player-Token` ; pas de comptes persistants. |
